...
runners.forEach(function(c1) {
    return fullName.push(`${c1.first_name} ${c1.last_name}`);
})
...
allCaps = runners.map(function(c2) {
    return c2.first_name.toUpperCase();
});
...
const newShirts = runners.filter(function(largeShirts) {
    return largeShirts.shirt_size === "L";
});

largeShirts = newShirts;
...
const taxdeduct = runners.reduce(function(acc, ticketPriceTotal) {
    return acc += ticketPriceTotal.donation;
}, 0);

ticketPriceTotal = taxdeduct;
...
// Problem 1
//find total donations from runners with size large t-shirts

const largeShirtDon = largeShirts.reduce(function(acc, num) {
    return acc += num.donation;
}, 0);

console.log(largeShirtDon)


// Problem 2
//create an new array that shows the email addresses of the people from the skinx company

let skinixEmail = [];

skinixEmail = runners.filter(function(P2) {
    return P2.company_name === "Skinix";
});

let skinmail = skinixEmail.map(function(p3) {
    return p3.email;
})

console.log(skinmail);


// Problem 3
//show donations over $100

let bigDona = [];

bigDona = runners.filter(function(donhun) {
    return donhun.donation > 100;
})
